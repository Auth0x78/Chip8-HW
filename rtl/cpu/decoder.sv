...
module decoder (
    ...
    output logic [ 3:0] reg_x_addr,
    output logic [ 3:0] reg_y_addr,
    output logic [ 7:0] imm_data,
    output logic [11:0] imm_addr
);
  ...
  assign alu_xy_op = (nibble3 === 4'h8) && ((instr[3:0] > 4'h0 && instr[3:0] < 4'h8) || instr[3:0] === 4'hE);  // Check if its a valid alu instruction in comb logic
  ...
  always_comb begin
    ...
    else if (br_skip_eq_imm 
              || br_skip_neq_imm 
              || load_x_imm 
              || alu_and_rand_imm
            ) 
    begin
      reg_x_addr = instr[11:8];
      imm_data   = instr[7:0];
    end 
    else if (br_skip_eq_reg || br_skip_neq_reg) begin
      reg_x_addr = instr[11:8];
      reg_y_addr = instr[7:4];
    end
    else if (ky_skip_xkey_press 
              || ky_skip_xkey_notpress
              || read_vx_mem_i 
              || store_V_reg 
              || store_bcd_of_x 
              || load_i_font 
